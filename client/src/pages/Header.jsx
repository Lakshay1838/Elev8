...
export default function Header() {
  ...
  useEffect(() => {
    axios.get("/createEvent").then((response) => {
  ...
  return (
    <div>
      <header className='flex py-2 px-4 sm:px-6 justify-between place-items-center'>
          ...
          <div  className='flex bg-white rounded py-2 px-4 w-1/3 gap-4 items-center shadow-md shadow-gray-200'>
          ...
          {!!user &&(
            ...
            {/* TODO: */}
              <nav className={`block ${isMenuOpen ? 'block' : 'hidden'} `}>
               ...
               {user?.role=="admin" && <Link className="flex hover:bg-background hover:shadow py-2 pt-3 pl-6 pr-8 rounded-lg" to={'/createEvent'} >
                  Host Event
                </Link>}
